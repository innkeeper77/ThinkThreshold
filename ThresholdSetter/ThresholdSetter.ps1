...
# Open Task scheduler, Action>Create Task. Give it a name (Eg:ThresholdsSetter), select "Run whether user is logged on or not", and "Run with the highest privileges".
# Under the Triggers tab, hit "New" at the bottom and under "Begin the task", select "At System Startup" Check the box for "Delay task for:" and set it to 1 minute (if desired). 
#
# When you have the task set, have it run powershell, silently, with this script as the argument. (You may have saved it in a different spot than me. Point to wherever this script is located)
# Under Actions, hit "New" at the bottom and set the action to "Start a program". Set the Program/script to Powershell.exe and under Add arguments, add the following: -ExecutionPolicy Bypass -WindowStyle Hidden C:\Startup\ThresholdSetter.ps1
#
...
# (For instructions on Task Scheduler, Thanks u/thebigbug from reddit, on https://www.reddit.com/r/Surface/comments/63orso/solution_for_undervolting_settings_not_sticking/ - I shamelessly copied much of it for the instructions here. )
# 
# You probably have to reboot to get this to work the first time, so once you have set everything up, run the script manually (just this once) and reboot. 
# Your thresholds should now be set!
#
# ----------------------------------------------------------------------------------------------------------------------------------

$BattSerial = "L4NJ75S03E7"
$StartP = "85"
$StopP = "90"
